...
Activate Serena by exact path for the code project actually in scope:

| Work area | Serena project path | Use Serena? |
| --- | --- | --- |
| `AI-Project-Manager` repo code/config | `D:/github/AI-Project-Manager` | Yes |
| OpenClaw runtime code | `D:/github/open--claw/open-claw` | Yes |
| DroidRun code | `D:/github/droidrun` | Yes |
| `D:/github/open--claw` repo-root governance/docs | No default Serena project | No by default; use targeted read/search fallback |

If Serena activation fails, state that explicitly, record FAIL evidence, and continue with targeted `rg`, `Glob`, and `ReadFile` work instead of assuming Serena is active.
