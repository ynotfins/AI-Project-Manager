...
**MCP tools used:**
- `sequential-thinking` (multi-step reasoning for complex plans)
...
- `filesystem_scoped` (file read/write within allowed paths)
...
- All configured MCP servers (selects dynamically):
  `Context7`, `Exa Search`, `firecrawl-mcp`, `github`, `filesystem_scoped`,
  `shell-mcp`, `playwright`, `Magic MCP`, `openmemory`, `sequential-thinking`,
  `serena`, `user-github`, `user-firestore-mcp`, `user-googlesheets`
...
**MCP tools used:**
- `shell-mcp` (build commands, CLI deploys)
- `github` (create branches, PRs, trigger workflows)
- `playwright` (verify deployed UI health)
- `filesystem_scoped` (read config, write deploy manifests)
