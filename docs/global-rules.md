...
|---|---|---|
| Reasoning / planning | sequential-thinking | Break into sub-steps manually |
...
- **sequential-thinking:** task has >5 connected steps, spans multiple files, involves migrations or refactors, or bug is ambiguous with multiple hypotheses.
...
- **Context7:** before changing behavior that depends on a third-party API, adopting a new dependency, or verifying framework function usage.
