...
- `.cursor/rules/openmemory.mdc` — OpenMemory retrieval and storage discipline
...
Serena must be activated by exact path for the codebase actually in scope, not by dashboard label:

| In-scope work | Activate Serena path | Notes |
| --- | --- | --- |
| `AI-Project-Manager` code/config | `D:/github/AI-Project-Manager` | Workflow/process layer project |
| OpenClaw runtime code | `D:/github/open--claw/open-claw` | Preferred Serena code project for OpenClaw |
| DroidRun code | `D:/github/droidrun` | Actuator-layer code project |

`D:/github/open--claw` repo root is not the primary Serena code project because it is docs-heavy and previously failed activation for code-intelligence use. For docs-only areas or unsupported roots, use targeted search/read tools and record Serena FAIL + fallback instead of assuming Serena is active.
